...
def format_date_warn(date_str, now=None):
    ...
    if (
        ...
        and date_str
        and date_str != "\u2014"
        and date_str not in _BAD_DATES
    ):
        _BAD_DATES.add(date_str)
        print(
            f"WARNING: unparseable date '{date_str}', using raw value", file=sys.stderr
        )
    return result
    ...
    def fetch(self):
        ...
        for row in reader:
            ...
            try:
                ...
                print(
                    f"WARNING: bad row for '{row['name']}', skipping", file=sys.stderr
                )
                continue

            if entity_count < 0:
                print(
                    f"WARNING: negative entity_count for '{row['name']}', skipping",
                    file=sys.stderr,
                )
                continue
            if not (0 <= max_pct <= 100) or not (0 <= min_pct <= 100):
                print(
                    f"WARNING: percentage out of range for '{row['name']}', skipping",
                    file=sys.stderr,
                )
                continue
    ...
    def extract_type_def_names(self):
        ...
        for filepath in self._walk_files(SCAN_EXTENSIONS):
            ...
            for match in re.finditer(
                r"^\s*\|\s*['\"]([a-zA-Z0-9_-]+)['\"]", content, re.MULTILINE
            ):
...
class ReportRenderer:
    ...
    def render(self, cats, ghosts):
        ...
        delete_title = (
            "\U0001f534 Not in This Project"
            if full
            else "\U0001f534 Fully Rolled Out (100%)"
        )
        gone_title = (
            "\U0001f480 Disabled \u2014 Not Here" if full else "\U0001f480 Disabled"
        )
        ...
        if full:
            summary_rows.append(
                ["\U0001f7e1 Needs Code Cleanup", len(cats["needs_cleanup"])]
            )
        summary_rows.append([delete_title, len(cats["safe_to_delete"])])
        if full:
            summary_rows.append(
                ["\u26ab Disabled \u2014 In Code", len(cats["disabled_in_code"])]
            )
        summary_rows.append([gone_title, len(cats["disabled_gone"])])
        if full:
            summary_rows.append(
                ["\u26a0\ufe0f Ghost (in code, not in DB)", len(ghosts)]
            )
        ...
        if full:
            section(
                ...
                lambda r, f: [
                    ...
                    enabled_cell(r),
                    pct_cell(r),
                    format_date_warn(r.max_expires, self.now),
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, default=int(os.environ.get("FF_DB_PORT", 63514))
    )
    parser.add_argument("--user", default=os.environ.get("FF_DB_USER"))
    ...
    if mode == "full":
        ...
        print(
            f"  Found {len(code_ff_names)} flag names in type definitions",
            file=sys.stderr,
        )

        all_tracked_names = {r.name for r in rows}.union(code_ff_names)
        print(
            f"Scanning codebase for references to {len(all_tracked_names)} flags...",
            file=sys.stderr,
        )
        references_index = scanner.build_references(all_tracked_names)
        print(
            f"  Indexed {sum(len(v) for v in references_index.values())} total references",
            file=sys.stderr,
        )
